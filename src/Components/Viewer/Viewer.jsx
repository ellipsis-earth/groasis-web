...
/*    let onSubatlasClick = (feature) => {
    ...
    GroasisUtility.getGroasisAreas(this.props.user, /*onSubatlasClick*/)
      .then(async groasisMaps => {
        ...
        if (maps.areas.length === 1)
        {
          this.setState({ groasisMaps: maps }, () => {this.onSelectMap(maps.areas[0].id)});
        }
        else
        {
   ...
   componentDidUpdate(prevProps) {
    if (this.props.mode !== prevProps.mode) {
      ...
      if (this.props.mode === ViewerUtility.identificationMode && !this.state.map)
      {
        this.onSelectMap();
      }
      else if (this.state.map && this.state.map.id && this.state.map.id === GroasisUtility.request.id)
      {
        this.setState({
          map: null,
          panes: panes,
  ...
  onWatchlistClick = async (feature, type = null) => {
    let extraCb = null;
    if (type)
    {
      extraCb = () => { this.onDataPaneAction(type) }
    }
  ...
  updatePolygons = (type, value) => {
    ...
    if (type && type === 'modeSwitch')
    {
      this.props.onModeChange(value, cb)
    }
    else
    {
      cb();
    ...
    else if (type === ViewerUtility.flyToType.givenElement)
    {
      ...
      this.getElementGeoJson()
        .then(geoJson => {
          ...
          if (flyToInfo.type === ViewerUtility.flyToType.standardTile) {
            flyToInfo.layer = ViewerUtility.standardTileLayerType;
          }
          else {
  ...
  getElementGeoJson = () => {
    ...
    let body = {
      mapId: map.id,
      type: type,
      timestamp: map.timestamps[this.state.timestampRange.end] ? map.timestamps[this.state.timestampRange.end].timestamp : 0
    };
  ...
  onPolygonChange = (removeDrawnPolygon, updateSelection, newProperties) => {
    ...
    if (updateSelection) {
      ...
      let selectedElement = this.state.selectedElement;
      selectedElement.feature.properties = properties
  ...
  calculateTimestamps = (map, selectedLayers) => {
    ...
    {
      let lowres = map.maps.find(x => ["4c450c42-1bf6-11e9-96ea-f0038c0f0121", "48d31d14-8cdd-401e-84a0-42941ad19dd6"].includes(x.dataSource.id));
